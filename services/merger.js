...
if preset == 'current_badge':
    ...
    if gt_p2: gt_parts.append((gt_p2, accent_rgba))
    ...
    draw_gradient_rect(img, 0, strip_top, W, strip_bottom, (0,0,0,185), (0,0,0,120))
    ...
    # rank rows — 9:16 portrait thumbnail
    THUMB_W   = 72
    THUMB_H   = 128
    THUMB_R   = 6
    ROW_PAD_X = 16
    ...
    for rank in range(1, total_ranks + 1):
        ...
        row_bg_color = (accent_rgba[0], accent_rgba[1], accent_rgba[2], 40) if is_active else (0,0,0,90)
        row_layer = Image.new('RGBA', (W, H), (0,0,0,0))
        row_draw  = ImageDraw.Draw(row_layer)
        row_draw.rounded_rectangle([ROW_PAD_X, cy_row, W-ROW_PAD_X, cy_row+row_h], radius=18, fill=row_bg_color)
        if is_active:
            row_draw.rounded_rectangle([ROW_PAD_X, cy_row, ROW_PAD_X+6, cy_row+row_h], radius=4, fill=(accent_rgba[0], accent_rgba[1], accent_rgba[2], 230))
        ...
        if thumb_path:
            try:
                ...
                bd.rounded_rectangle([thumb_x-2, thumb_y-2, thumb_x+THUMB_W+2, thumb_y+THUMB_H+2], radius=THUMB_R+2, outline=rank_color, width=3)
        ...
        # circle badge with rank number
        badge_r = 26 if is_active else 20
        badge_cx = text_start_x + badge_r
        badge_cy = cy_row + row_h // 2
        badge_layer = Image.new('RGBA', (W,H), (0,0,0,0))
        bd2 = ImageDraw.Draw(badge_layer)
        bd2.ellipse([badge_cx-badge_r, badge_cy-badge_r, badge_cx+badge_r, badge_cy+badge_r], fill=rank_color)
        ...
        if title and is_active:
            ...
            ttl_lh    = int(pro_act_ttl_size * 1.3)
...
async function mergeVideos({ videoFiles, sourcesMeta = [], workDir, jobId, heading, ranking, audioOpts, jobLog, speeds = [] }) {
  ...
  // pro_ranking: extract first frame thumbnail from each cropped clip
  let thumbnailPaths = [];
